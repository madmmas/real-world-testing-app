...
- **E2E-PUB-06** Sign in as `buyer` / `Passw0rd!` (wait for frictionless Altcha when `auth.altcha` is on) → lands authenticated; refresh stays logged in (sessionStorage refresh token).
- **E2E-PUB-07** Wrong password → error; after 3 consecutive failures UI shows **Interactive bot check** (checkbox Altcha) when Altcha is on.
...
- **E2E-PUB-18** `ALTCHA_ENABLED=false`: public sign-in/up have no Altcha widget and succeed without a captcha payload.
